...
module.exports = class AuthorizationUtil {
  ...
  static createJWT (userId, email, isAdmin) {
    const token = jwt.sign({ userId, email, isAdmin }, this.getJWTKey(), {
      algorithm: 'HS256'
    })
   ...
   * @function
   * @param {Number} userId - User his id (likely coming from the database)
   * @returns {Object} - {userId,email,isAdmin} or undefined if the key is not valid
   */
  ...
  static isAuthenticatedAsUser (req, res, next) {
    ...
    if (jwtPayload !== undefined) {
      req.user = new User(jwtPayload.userId, jwtPayload.email, undefined, undefined, undefined, undefined, undefined, undefined, jwtPayload.isAdmin)
      return next()
    } else {
